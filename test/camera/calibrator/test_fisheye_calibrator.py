...
from ..util import *
from lovely_utils.camera.calibrator.fisheye_calibrator import FisheyeCalibrator
from lovely_utils.camera.detector.chessboard_detector import ChessboardDetector


def test_chessboard_calibrate_intrinsic(generate_fisheye_calibrator_intrinsic_params):
    fisheye_calibrator_intrinsic_params = generate_fisheye_calibrator_intrinsic_params

    dir_save_detect_result = Path(fisheye_calibrator_intrinsic_params["dir_save_detect_result"])
    # dir_save_detect_result = None
    dir_imgs = Path(fisheye_calibrator_intrinsic_params["dir_calib_images"])
    path_imgs = list(dir_imgs.glob("image*.jpg"))

    chessboard_detector = ChessboardDetector(chessboard_size=(6, 9))
    fisheye_calibrator = FisheyeCalibrator(intrinsic_detector=chessboard_detector)

    rms, mtx, dist, rvecs, tvecs = fisheye_calibrator.calibrate_intrinsic(
        images=path_imgs, dir_save_detect_result=dir_save_detect_result,remove_unvalid_image=False
    )
    assert np.allclose(mtx, fisheye_calibrator_intrinsic_params["K"], atol=1e-3)
    assert np.allclose(dist, fisheye_calibrator_intrinsic_params["D"], atol=1e-3)
